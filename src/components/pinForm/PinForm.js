...
const PinForm = () => {
  ...
  useEffect(() => {
    if (localStorageUser) {
      fetch(`http://localhost:5000/api/users/${localStorageUser._id}/boards`) //http://localhost:5000/api/users/21/boards ruta correcta
        .then((promise) => {
  ...
  const handleSubmit = () => {
    ...
    fetch("http://localhost:5000/api/pins", options).then((response) =>
      response.json()
    );
